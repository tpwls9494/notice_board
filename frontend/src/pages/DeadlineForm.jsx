...
function DeadlineForm() {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => deadlinePostsAPI.createPost(data),
    onSuccess: (response) => {
      alert('마감기한 게시글이 작성되었습니다.')
      navigate(`/deadline/${response.data.id}`)
    },
    onError: (error) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => deadlinePostsAPI.updatePost(id, data),
    onSuccess: () => {
      alert('게시글이 수정되었습니다.')
  ...
  for (let h = 0.5; h <= 24; h += 0.5) {
    const label = h < 1
      ...
      : h === Math.floor(h)
        ? `${h}시간`
        : `${Math.floor(h)}시간 ${(h % 1) * 60}분`
    deadlineOptions.push({ value: h, label })
  }
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-up">
      {/* Back Navigation */}
      <Link
        to={isEdit ? `/deadline/${id}` : '/deadline'}
      ...
      <div className="card overflow-hidden">
        <div className="px-6 sm:px-8 py-5 border-b border-ink-100">
          <h1 className="font-display text-xl font-bold text-ink-950 tracking-tight text-balance">
            {isEdit ? '마감기한 게시글 수정' : '새 마감기한 게시글'}
          </h1>
          <p className="text-sm text-ink-400 mt-1">
            {isEdit ? '내용을 수정하고 저장하세요' : '마감 시간이 있는 게시글을 작성하세요'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-5">
          ...
          {!isEdit && (
            <div>
              <label htmlFor="deadline" className="block text-sm font-semibold text-ink-700 mb-2">
                마감 시간
              </label>
              <select
                id="deadline"
                value={deadlineHours}
                onChange={(e) => setDeadlineHours(parseFloat(e.target.value))}
              ...
              <p className="text-xs text-ink-400 mt-1.5">
                게시글 작성 시점부터 마감까지의 시간입니다 (최대 24시간)
              </p>
            </div>
          )}
